    ...
    try:
        ...
        try:
            ...
            if not bool(modules.get("kg_extract_enabled", True)):
                ...
                #
                # 总超时保护：KG 抽取是对知识图谱补数据，对 RAG 检索非必需。
                # 之前没有总超时，101 个 chunk 中多个命中 LLM 超时后，底层
                # to_thread 线程无法被 wait_for 取消，会泄漏占满默认
                # ThreadPoolExecutor，导致整个 KG 阶段永久 hang，文档永远停在 processing。
                kg_total_timeout_s = int(params.get("kg_total_timeout_s") or 180)  # 默认 3 分钟
                try:
                    ...
                    if loop.is_running():
                        ...
                        loop.run_until_complete(
                            asyncio.wait_for(build_kg_for_chunks(), timeout=kg_total_timeout_s)
                        )
                except RuntimeError:
                    # 没有 loop
                    asyncio.run(
            ...
            if isinstance(e, asyncio.TimeoutError):
                logger.warning(
                    f"知识图谱构建总超时（{kg_total_timeout_s}s），已跳过剩余 chunk 继续向量化 - 文档ID: {doc_id}"
                )
            else:
                logger.error(f"知识图谱构建失败 (非致命错误): {e}", exc_info=True)
            # 不中断主流程：KG 是补数据，向量化才是 RAG 检索所需
                ...
                try:
                    ...
                    if isinstance(knowledge_space_id, str):
                        try:
                            ...
                            # 不是合法 ObjectId（比如传的是知识空间名称），走名称兜底
                            space_doc = db["knowledge_spaces"].find_one({"name": knowledge_space_id})
                    else:
                        space_doc = db["knowledge_spaces"].find_one({"_id": knowledge_space_id})
                    ...
                    logger.warning(f"查找知识空间失败: {str(oid_error)}, 知识空间ID: {knowledge_space_id}, 文档ID: {doc_id}")
...
async def upload_document(
    ...
    try:
        ...
        # 知识空间 ID 可选：留空时走后端默认策略（写入 default_knowledge 集合）
