...
class ChunkRepository:
    ...
    def _sanitize_for_bson(value: Any) -> Any:
        """
        递归清洗 metadata，把 PyMongo BSON 编码器不支持的 Python 类型转成可编码类型。
        - set/tuple → list
        - dict 的 value 递归清洗
        - list 的元素递归清洗
        其他类型原样返回。防止因单个字段导致整条 chunk 写入失败。
        """
        if isinstance(value, (set, tuple)):
            return [ChunkRepository._sanitize_for_bson(v) for v in value]
        if isinstance(value, dict):
            return {k: ChunkRepository._sanitize_for_bson(v) for k, v in value.items()}
    ...
    def create_chunk(
        ...
        chunk = {
            ...
            "text": text,
            # 入库前递归清洗，防止 set/tuple 等导致整条 chunk 写入失败
            "metadata": self._sanitize_for_bson(metadata or {}),
            "created_at": beijing_now()
        }
